...
void CListExHdr::SetColor(const LISTEXCOLORS& lcs)
{
	...
	m_clrHglActive = lcs.clrHdrHglInact;// lcs.clrHdrHglAct;
...
void CListExHdr::OnDrawItem(CDC* pDC, int iItem, CRect rcOrig, BOOL bIsPressed, BOOL bIsHighlighted)
{
	...
	const COLORREF clrBk{ RGB(106, 153, 207)};; 
	//rcOrig.left -= 1;
	//rcOrig.right += 2;
...
void CListEx::DrawItem(LPDRAWITEMSTRUCT pDIS)
{
	...
	case ODA_DRAWENTIRE:
	{
		...
		for (auto iColumn = 0; iColumn < iColumns; ++iColumn) {
			...
			if (iColumn == 0 || iColumn == iColumns - 1)
			{
				rcBounds.left += 1;
				rcBounds.right -= 1;
			}
...
void CListEx::OnLButtonDblClk(UINT nFlags, CPoint point)
{
	...
	//if (SubItemHitTest(&m_htiInPlaceEdit) == -1)
	//	return CMFCListCtrl::OnLButtonDblClk(nFlags, point);

	//if (!GetHeaderCtrl().IsColumnEditable(m_htiInPlaceEdit.iSubItem))
	//	return CMFCListCtrl::OnLButtonDblClk(nFlags, point);
...
void CListEx::TtRowShow(bool fShow, UINT uRow)
{
	if (fShow) {
		...
		m_stWndTtRow.SendMessageW(TTM_TRACKPOSITION, 0, static_cast<LPARAM>(MAKELONG(ptScreen.x - 5, ptScreen.y - 10)));
